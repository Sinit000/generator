...
<style>
    ...
    .en[data="change_dayoff"]::before {
        content: 'Notification Change'

    }

    .kh[data="change_dayoff"]::before {
        content: 'ផ្លាស់ប្តូរថ្ងៃឈប់ឈម្រាក'
    }
...
<div class="main-content">
    <section class="section">
        <div class="section-header">
            <h1 data="change_dayoff" id="title-family" class="en"></h1>
...
checkLanguage()
    ...
    $(document).ready(function() {
        ...
        $('#user-table').DataTable({
            ...
            processing: true,
            serverSide: true,
            ajax: "{{url('admin/approve/changedayoffs')}}",
            columns: [{
                    data: 'DT_RowIndex',
                    name: 'DT_RowIndex',
                    orderable: false,
        ...
        $('#btn-save').click(function() {
            ...
            if (state == "update") {
                ...
                var id = $('#id').val();
                type = "PUT";
                ajaxurl = "{{ url('admin/approve/changedayoffs/update') }}" + '/' + id;
                console.log(ajaxurl);
                console.log(id);
            }
        ...
        $(document).on('click', '#editBtn', function() {
            ...
            console.log(id);
            $.get("{{ url('admin/approve/changedayoffs/edit') }}" + '/' + id, function(data) {
            ...
            swal("Warning!", "Are you sure you want to delete?", "warning", {
                ...
                switch (value) {
                    case "ok":
                        $.ajax({
                            type: "DELETE",
                            url: "{{ url('admin/dayoff/delete') }}" + '/' + id,
                            success: function(data) {
                                if (data.code == 0) {
                                    $('#user-table').DataTable().draw(false);
                                    $('#user-table').DataTable().on('draw', function() {
