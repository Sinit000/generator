        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form id="company-form">
                    ...
                    <div class="form-group">
                        <label for="nip">Province <sup class="text-danger">*</sup></label>
                        <select class="form-control select2" id="province_id" name="province_id" style="width: 100%;">
                            <option value="">Choose Province</option>
                        </select>
                        <div class="invalid-feedback" id="valid-province_id"></div>
                    </div>
...
<div class="main-content">
    ...
    <section class="section">
        ...
        <div class="section-header">
            <h1 data="district" id="title-family" class="en"></h1>
                        ...
                        <table class="table table-sm table-hover table-bordered" id="user-table">
                            <thead class="thead-light">
                                <tr>
                                    ...
                                    <th>
                                        <p class="en" data="province" id="subtitle-family"></p>
                                    </th>
...
<script>
    ...
    $(document).ready(function() {
        ...
        $('#btn-add').click(function() {
            ...
            $.ajax({
                type: "GET",
                url: "{{url('admin/district/componet')}}",
                // data:"",
                dataType: "json",
                success: function(response) {
                    ...
                    if (response.status == 404) {
                        ...
                        var selOpts = "";
                        for (i = 0; i < response.province.length; i++) {

                            var id = response.province[i]['id'];
                            var val = response.province[i]['name'];
                            selOpts += "<option value='" + id + "'>" + val + "</option>";
                        }
                        $('#province_id').append(selOpts);
            ...
            $('.close').click(function() {
                ...
                $('#province_id').find('option').remove().end().append('<option value="">Choose Province</option>').val('');
        ...
        $('#btn-save').click(function() {
            ...
            var type = "POST";
            var ajaxurl = "{{url('admin/district/store')}}";
            ...
            if (state == "update") {
                ...
                var id = $('#id').val();
                type = "POST";
                ajaxurl = "{{ url('admin/district/update') }}" + '/' + id;
                console.log(ajaxurl);
            }
                ...
                error: function(data) {
                    try {
                        if (state == "save") {
                            ...
                            if (data.responseJSON.errors.province_id) {
                                $('#password').removeClass('is-valid').addClass('is-invalid');
                                $('#valid-province_id').removeClass('valid-feedback').addClass('invalid-feedback');
                                $('#valid-province_id').html(data.responseJSON.errors.province_id);
                            }
        ...
        $(document).on('click', '#editBtn', function() {
            ...
            console.log(id);
            $.get("{{ url('admin/district/edit') }}" + '/' + id, function(data) {
                $('#company-form').find('.form-control').removeClass('is-invalid is-valid');
                $('#id').val(data.data.id);
                ...
                $.each(data.province, function(key, value) {
                    time = $('select[name="province_id"]')
                        .append(`<option value="${value.id}" ${value.id == data.data.province_id ? 'selected' : ''}>${value.name}</option>`)
