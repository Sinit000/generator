            ...
            <div class="modal-body">
                <form id="company-form">
                    ...
                    <div class="form-group">
                        <label for="nip">Leave type <sup class="text-danger">*</sup></label>
                        <input type="text" class="form-control" id="leave_type" name="leave_type" placeholder="Enter leavetye..." autocomplete="off">
                        <div class="invalid-feedback" id="valid-name"></div>
                    </div>
                    <div class="form-group">
                        <label for="location_id">Parent leave</label>
                        <select class="form-control select2" id="parent_id" name="parent_id" style="width: 100%;">
                            <option value="">Choose leavetype</option>
                        </select>
                        <div class="invalid-feedback" id="valid-parent_id"></div>
                    </div>
                    <div class="form-group">
                        <label for="name">Duration </label>
                        <input type="text" class="form-control" id="duration" name="duration" placeholder="Enter duration..." autocomplete="off">
                        <div class="invalid-feedback" id="valid-duration"></div>
                    </div>
                    <div class="form-group">
...
<script>
    $(document).ready(function() {
        ...
        $('#btn-add').click(function() {
            ...
            $.ajax({
                type: "GET",
                url: "{{url('admin/leavetype/componet')}}",
                // data:"",
                dataType: "json",
                success: function(response) {
                    ...
                    if (response.status == 404) {
                        ...
                        for (i = 0; i < response.data.length; i++) {
                            console.log(response.data[i]['leave_type']);
                            var id = response.data[i]['id'];
                            var val = response.data[i]['leave_type'];
                            selOpts += "<option value='" + id + "'>" + val + "</option>";

                        }

                        $('#parent_id').append(selOpts);
            ...
            $('.close').click(function() {
                ...
                $('#parent_id').find('option').remove().end().append('<option value="">Chooose leavetype</option>').val('');
        ...
        $('#btn-save').click(function() {
            ...
            var type = "POST";
            var ajaxurl = "{{url('admin/leavetype/store')}}";
            ...
            if (state == "update") {
                ...
                var id = $('#id').val();
                type = "PUT";
                ajaxurl = "{{ url('admin/leavetype/update') }}" + '/' + id;
                console.log(ajaxurl);
            }
            ...
            $.ajax({
                ...
                error: function(data) {
                    try {
                        if (state == "save") {
                            if (data.responseJSON.errors.leave_type) {
                                $('#leave_type').removeClass('is-valid').addClass('is-invalid');
                                $('#valid-name').removeClass('valid-feedback').addClass('invalid-feedback');
                                $('#valid-name').html(data.responseJSON.errors.leave_type);
                            }
                            if (data.responseJSON.errors.duration) {
                                $('#duration').removeClass('is-valid').addClass('is-invalid');
                                $('#valid-duration').removeClass('valid-feedback').addClass('invalid-feedback');
                                $('#valid-duration').html(data.responseJSON.errors.duration);
                            }
        ...
        $(document).on('click', '#editBtn', function() {
            ...
            console.log(id);
            $.get("{{ url('admin/leavetype/edit') }}" + '/' + id, function(data) {
                $('#company-form').find('.form-control').removeClass('is-invalid is-valid');
                // show data on modal
                // $('#name').val(data.nip);
                $('#id').val(data.data.id);
                $('#leave_type').val(data.data.leave_type);
                ...
                $.each(data.type, function(key, value) {
                    ...
                    selOpts = $('select[name="parent_id"]').append(`<option value="${value.id}" ${value.id == data.data.parent_id? 'selected' : ''}>${value.leave_type}</option>`)
                });
                // $('.editModal').append(selOpts);
                $('#formModal').find('#parent_id').append(selOpts)
            ...
            swal("Warning!", "Are you sure you want to delete?", "warning", {
                ...
                switch (value) {
                    case "ok":
                        $.ajax({
                            type: "DELETE",
                            url: "{{ url('admin/leavetype/delete') }}" + '/' + id,
                            success: function(data) {
                                if (data.code == 0) {
                                    $('#user-table').DataTable().draw(false);
                                    $('#user-table').DataTable().on('draw', function() {
