            ...
            <div class="modal-body">
                <form id="company-form">
                    ...
                    <div class="form-group">
                        <label for="timetable_name">Name <sup class="text-danger">*</sup></label>
                        <input type="text" class="form-control" id="timetable_name" name="timetable_name" placeholder="Enter timetable name..." autocomplete="off">
                        <div class="invalid-feedback" id="valid-name"></div>
                    </div>
                    <div class="form-group">
                        <label for="on_duty_time">Time in <sup class="text-danger">*</sup></label>
                        <div class="bootstrap-timepicker">
                            <input type="time" class="form-control timepicker" id="on_duty_time" name="on_duty_time">
                        </div>
                        <div class="invalid-feedback" id="valid-on_duty_time"></div>
                    </div>
                    <div class="form-group">
                        <label for="off_duty_time">Time out <sup class="text-danger">*</sup></label>
                        <div class="bootstrap-timepicker">
                            <input type="time" class="form-control timepicker" id="off_duty_time" name="off_duty_time">
                        </div>

                        <div class="invalid-feedback" id="valid-off_duty_time"></div>
                    </div>
                    <div class="form-group">
                        <label for="name">Late minutes </label>
                        <input type="text" class="form-control" id="late_minute" name="late_minute" placeholder="Enter late minutes..." autocomplete="off">
                        <div class="invalid-feedback" id="valid-name"></div>
                    </div>
                    <div class="form-group">
...
<script>
    $(document).ready(function() {
        ...
        $('#btn-save').click(function() {
            // save data state
            var formData = {
                timetable_name: $('#timetable_name').val(),
                on_duty_time: $('#on_duty_time').val(),
                off_duty_time: $('#off_duty_time').val(),
                late_minute: $('#late_minute').val(),
                early_leave: $('#early_leave').val(),
            };

            var state = $('#btn-save').val();

            var type = "POST";
            var ajaxurl = "{{url('admin/timetable/store')}}";
            ...
            if (state == "update") {
                ...
                var id = $('#id').val();
                type = "PUT";
                ajaxurl = "{{ url('admin/timetable/update') }}" + '/' + id;
                console.log(ajaxurl);
            }
        ...
        $(document).on('click', '#editBtn', function() {
            ...
            console.log(id);
            $.get("{{ url('admin/timetable/edit') }}" + '/' + id, function(data) {
                $('#company-form').find('.form-control').removeClass('is-invalid is-valid');
                // show data on modal
                // $('#name').val(data.nip);
                $('#id').val(data.id);
                $('#timetable_name').val(data.timetable_name);
                $('#on_duty_time').val(data.on_duty_time);
                $('#off_duty_time').val(data.off_duty_time);
                $('#late_minute').val(data.late_minute);
                $('#early_leave').val(data.early_leave);
            ...
            swal("Warning!",
                ...
                switch (value) {
                    case "ok":
                        $.ajax({
                            type: "DELETE",
                            url: "{{ url('admin/timetable/delete') }}" + '/' + id,
                            success: function(data) {
                                if (data.code == 0) {
                                    $('#user-table').DataTable().draw(false);
                                    $('#user-table').DataTable().on('draw', function() {
