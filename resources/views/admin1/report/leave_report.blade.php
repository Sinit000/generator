...
<script type="text/javascript">
    $(document).ready(function() {
        ...
        function getData(startDate, endDate, id) {
            ...
            $.ajax({
                type: "GET",
                url: "{{ url('admin/report/leaves') }}" + '/' + id + '/' + startDate + '/' + endDate + '/' + 'false',
                dataType: 'json',
                beforeSend: function() {
                ...
                success: function(data) {
                    ...
                    if(data.data.length==0){
                        ...
                        if (item.send_status == "true") {
                            ...
                            var element = `<tbody>
                                <tr>
                                    <td  class="text-center"><input type="checkbox" name="country_checkbox" data-id="${item.leave_id}"></td>
                                    <td  class="text-center">${index+1}</td>
                                    <td  class="text-center">${item.checkin_date}</td>
                                    <td class="text-center">${item.user_name}</td>
                                    <td class="text-center">${item.typename}</td>
        ...
        $('#btn_export').on('click', function() {
            ...
            var url = "{{URL::to('admin/report/leaves')}}" + '/' + userId + '/' + fromDate + '/' + toDate + '/' + 'true';
            console.log(userId);
        ...
        function toggledeleteAllBtn() {
            if ($('input[name="country_checkbox"]:checked').length > 0) {
                $('button#deleteAllBtn').text('Confirm (' + $('input[name="country_checkbox"]:checked').length + ')').removeClass('d-none');
            } else {
